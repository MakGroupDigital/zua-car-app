...
import { ArrowLeft, MapPin, Loader2, Car, Phone, Mail, Star, Clock } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast({
        ...
        description: 'Veuillez vous connecter pour ajouter un garage',
      });
      router.push('/login');
      return;
    }
    ...
    try {
      const garageId = `garage_${user.uid}_${Date.now()}`;
      ...
      if (formData.services) {
        garageData.services = formData.services.split(',').map(s => s.trim()).filter(s => s.length > 0);
      }
      ...
      toast({
        title: 'Garage ajouté ! 🎉',
        description: 'Votre garage est maintenant visible sur la carte',
      });
      ...
      toast({
        variant: 'destructive',
        title: 'Erreur',
        description: error.message || 'Impossible d\'ajouter le garage',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-accent/5 pb-20">
      <div className="sticky top-0 z-10 bg-gradient-to-r from-primary/10 via-background/90 to-accent/10 backdrop-blur-xl border-b border-primary/20 p-4 flex items-center gap-4 shadow-lg">
        <Button variant="ghost" size="icon" onClick={() => router.push('/garages')} className="hover:bg-primary/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="name" className="text-primary font-semibold">
                  Nom du garage <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="name"
                  name="name"
                  ...
                  placeholder="Ex: Garage Auto Express"
                  required
              ...
              {/* Services */}
              <div className="space-y-2">
                <Label htmlFor="services" className="text-primary font-semibold">
                  Services (optionnel)
                </Label>
                <Textarea
                  id="services"
                  ...
                  onChange={handleInputChange}
                  placeholder="Ex: Réparation, Entretien, Pneus (séparés par des virgules)"
                  className="border-primary/20 focus:border-primary min-h-[80px]"
                />
                <p className="text-xs text-muted-foreground">
                  Séparez les services par des virgules
                </p>
              </div>
